...
#'
#'
mod_import_dt_ti_ui <- function(id){
  ns <- NS(id)
  tagList(
    fluidRow(
      bs4Dash::box(title = tagList(shiny::icon("upload"), "Source"),
                   ...
                   conditionalPanel("input.Idd004==1",h6('Use the example database to try the different modules of Mr. Bean'), ns = ns),
                   conditionalPanel("input.Idd004==2",h6('Import external data preferably csv/txt files.'), ns = ns)
      ),

      column(width = 3,
             conditionalPanel("input.Idd004==2", ns = ns,
    ...
    fluidRow(bs4Dash::box(collapsed = F,maximizable = T,closable = F,
                          ...
                          width = 12,title =  tagList(shiny::icon("file-import"), "Data1"),status = "success",solidHeader = FALSE,collapsible = TRUE)
    ),br()
  )
}
...
#'
#' @noRd
mod_import_dt_ti_server <- function(input, output, session){
  ns <- session$ns
  ...
  observeEvent(sum(input$config)==0, shinyjs::toggle("bms_config",anim = TRUE,time = 1,animType = "fade"))
  ...
  observeEvent(input$subset==F,{
    shinyjs::toggle("varsubset",anim = TRUE,time = 1,animType = "fade")
  })

  observeEvent(input$varsubset,{
    shinyjs::toggle("levelessub",anim = TRUE,time = 1,animType = "fade",condition = input$varsubset!="")
    req(input$varsubset)
